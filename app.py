...
def load_data():
    try:
        ...
        def parse_date(r):
            ...
            return r["생성일_dt"]
        df["날짜"] = df.apply(parse_date, axis=1)
        return df, None
...
def render_dashboard(df):
    ...
    fdf = df.copy()
    if len(date_range) == 2: fdf = fdf[(fdf["날짜"].dt.date >= date_range[0]) & (fdf["날짜"].dt.date <= date_range[1])]
    ...
    chart_df = fdf.copy()
    if t_unit == "일간": chart_df["g"] = chart_df["날짜"].dt.strftime("%y.%m.%d")
    elif t_unit == "월간": chart_df["g"] = chart_df["날짜"].dt.to_period("M").dt.start_time.dt.strftime("%y년 %m월")
    else: chart_df["g"] = chart_df["날짜"].dt.to_period("W").dt.start_time.dt.strftime("%y.%m.%d") + "(주)"
    ...
    if st.session_state.selected_points:
        ...
        st.markdown(f"#### 🎯 선택된 기간 상세 성과 ({', '.join(st.session_state.selected_points)})")
        if st.button("선택 초기화 ✖️"): 
        ...
        if not detail_df.empty:
            ...
            st.dataframe(detail_df[["날짜", "utm_content", "utm_campaign", "utm_source", "UV", "결제완료", "CVR", "결제금액"]].reset_index(drop=True), use_container_width=True, hide_index=True)
    ...
    st.dataframe(fdf.sort_values("날짜", ascending=False).drop(columns=["g","생성일_dt","CVR_num","결제금액_num"], errors='ignore'), use_container_width=True, hide_index=True)
    ...
    with st.container(border=True):
        ...
        if url and src and med and cam and cnt:
            ...
            if st.button("🚀 구글 시트에 추가", use_container_width=True):
                st.success("데이터가 성공적으로 전송되었습니다 (데모)"); st.cache_data.clear()
